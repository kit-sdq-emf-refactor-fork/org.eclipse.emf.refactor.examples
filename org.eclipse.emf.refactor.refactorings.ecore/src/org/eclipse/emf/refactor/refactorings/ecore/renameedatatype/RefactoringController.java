...
public final class RefactoringController implements IController{
	...
	private Runnable applyRefactoring() {
		return new Runnable() {				
			...
			public void run() {
				// begin custom code
				org.eclipse.emf.ecore.EDataType selectedEDataType = 
					(org.eclipse.emf.ecore.EDataType) dataManagement.
							getInPortByName(dataManagement.SELECTEDEOBJECT).getValue();
				String newName = 
					(String) dataManagement.getInPortByName("newName").getValue();
				selectedEDataType.setName(newName);
				// end custom code
			}
		};
	}
	...
	public final class InternalRefactoringProcessor extends 
		...
		public RefactoringStatus checkFinalConditions(){
				...
				if (selectedEDataType.getName().equals(newName)) {
					result.addFatalError("The new name is equal to the old name!");
				}
				// final check 2: the new name must be a valid EDataType name
				if (! RefactoringHelper.isValidEDataTypeName(newName)) {
					String message = 
							RefactoringHelper.getReasonForInvalidEDataTypeName(newName);
					result.addFatalError(message);
				}
				// final check 3: the containing EPackage must not contain an EClassifier
				// with the specified name
				if (RefactoringHelper.
						containsEClassifier(selectedEDataType.getEPackage(), newName)) {
